...
/**
 * Owns the handles one client issued. Each client supplies the payload type it needs to act on a
...
export function createHandleRegistry<DbcPayload, TracePayload>(): HandleRegistry<
	DbcPayload,
	TracePayload
> {
	type AnyPayload = DbcPayload | TracePayload;
	...
	return {
		issue(kind, payload) {
			...
			entries.set(token, { closed: false, kind, payload });
