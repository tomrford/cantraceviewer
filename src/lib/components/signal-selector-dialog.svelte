<script lang="ts">
	...
	const closeButtonClass =
		'absolute top-[6.5px] left-[7px] flex size-8 shrink-0 items-center justify-center rounded-md text-destructive transition-[background-color,color,box-shadow,scale] hover:bg-destructive/10 hover:text-destructive focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-hidden active:scale-[0.96]';
...
<Popover.Content
	align="start"
	sideOffset={-32}
	class="relative grid h-[min(36rem,calc(100vh-5rem))] w-[min(26rem,calc(100vw-1rem))] -translate-x-2 -translate-y-2 grid-rows-[auto_auto_minmax(0,1fr)] gap-3 overflow-hidden rounded-lg border border-border/70 bg-popover/90 p-4 text-popover-foreground backdrop-blur-md"
