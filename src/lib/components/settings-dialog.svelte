<script lang="ts">
	...
	const closeButtonClass =
		'absolute top-[6.5px] right-[7px] flex size-8 items-center justify-center rounded-md text-destructive transition-[background-color,color,box-shadow,scale] hover:bg-destructive/10 hover:text-destructive focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-hidden active:scale-[0.96]';
...
>
	<div class="grid h-8 grid-cols-[1fr_auto_1fr] items-center">
		...
		<Popover.Close class={closeButtonClass} aria-label="Close settings">
			<XIcon class="size-4" />
		</Popover.Close>
	</div>
